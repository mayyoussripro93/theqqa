...
Route::group([
	...
	Route::group([
        ...
        CRUD::resource('Bank transfer', 'BanktransferController');
        ...
        Route::get('verify/user/booking/{id}/{post_id}/{package_id}/{payment_id}/resend/email', 'PostController@SendBookingEmail');
...
Route::group([
	...
	Route::group(['middleware' => ['web']], function ($router) {
		// HOMEPAGE
        Route::get(LaravelLocalization::transRoute('routes.page'), 'API\PageController@index');
		Route::get('/', 'HomeController@index');
		Route::get(LaravelLocalization::transRoute('routes.countries'), 'CountriesController@index');

        Route::post('post/loaddata', 'HomeController@loadDataAjax');
        
        Route::post('paytab/savedata', 'HomeController@paytabDataAjax');
        Route::get('paytab/savedata', 'HomeController@paytabDataAjax');
         
        Route::get('paytab/servce/savedata', 'Post\PaymentController@paytabservceDataAjax');
        Route::post('paytab/servce/savedata', 'Post\PaymentController@paytabservceDataAjax');
         
       Route::post('paytab/booking/savedata', 'HomeController@bookingPaytabDataAjax');
              Route::get('paytab/booking/savedata', 'HomeController@bookingPaytabDataAjax');
		// AUTH
		Route::group(['middleware' => ['guest', 'prevent.back.history']], function () {
            ...
            Route::get('register/Company', 'Auth\RegisterController@showregisterCompanyForm');
            Route::post(('register/Company'), 'Auth\RegisterController@registerCompany');

            Route::get('register/shop', 'Auth\RegisterController@showregisterCompanyForm');
            Route::post(('register/shop'), 'Auth\RegisterController@registershop');

            Route::get('register/exhibition', 'Auth\RegisterController@showregisterCompanyForm');
            Route::post(('register/exhibition'), 'Auth\RegisterController@registerexhibition');
		...
		Route::group(['namespace' => 'Post'], function ($router) {
			...
			// $router->pattern('slug', '.*');
			$router->pattern('slug', '^(?=.*)((?!\/).)*$');
            Route::get('posts/req_doc/{tmpToken?}', 'CreateController@getFormReqDocuments');
			...
			Route::group(['middleware' => 'auth'], function ($router) {
                ...
                Route::get('post/{id}/paymentservice', 'PaymentController@getFormmogaz');
                Route::post('post/{id}/paymentservice', 'PaymentController@postFormmogaz');
                Route::get('contactfor/{id}/booking_car', 'PaymentController@booking_car');
                Route::post('contactfor/{id}/booking_car', 'PaymentController@booking_post_car');
				// Payment Gateway Success & Cancel
		...
		Route::group(['prefix' => 'ajax'], function ($router) {
            ...
            Route::post('countries/{countryCode}/posts/autocomplete', 'Ajax\PostController@searchedposts');
        ...
        //mogaz PAGE
        Route::get('contactfor/mogaz', 'PageController@contact_mogaz');
        Route::post('contactfor/mogaz', 'PageController@contactPost_mogaz');

        Route::get('contactfor/ownership', 'PageController@contact_ownership');
        Route::post('contactfor/ownership', 'PageController@contactPost_ownership');

        Route::get('contactfor/checking', 'PageController@contact_checking');
        Route::post('contactfor/checking', 'PageController@contactPost_checking');

        Route::get('contactfor/shipping', 'PageController@contact_shipping');
        Route::post('contactfor/shipping', 'PageController@contactPost_shipping');

        Route::get('contactfor/estimation', 'PageController@contact_estimation');
        Route::post('contactfor/estimation', 'PageController@contactPost_estimation');

        Route::get('contactfor/maintenance', 'PageController@contact_maintenance');
//        Route::get('contactfor/form/maintenance/{lat}/{lng}', 'PageController@contact_maintenance_ajax');
        Route::post('contactfor/maintenance', 'PageController@contactPost_maintenance');
        Route::get('city/get/{id}', 'PageController@get_cities');
        Route::get('contactfor/maintenance/yes', 'PageController@contact_maintenance_yes');
