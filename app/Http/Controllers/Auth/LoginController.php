...
class LoginController extends FrontController
{
    ...
    public function login(LoginRequest $request)
    {
        ...
        $userlog = \DB::select("select * from theqqausers where email='$var'");
        ...
        if(!empty($userlog))
        {
         ...
         if($userlog['0']->user_type_id == 6){
             return redirect($this->loginPath)->withErrors(['error' => trans('auth.active_exp')])->withInput();
         }

         if( $userlog['0']->verified_email == 1){
             return redirect($this->loginPath)->withErrors(['error' => trans('auth.error_detail')])->withInput();
         }

            return redirect($this->loginPath)->withErrors(['error' => trans('auth.active')])->withInput();
