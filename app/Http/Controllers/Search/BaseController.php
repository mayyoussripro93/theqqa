    ...
    public function commonQueries()
    {
        ...
        if (config('settings.listing.left_sidebar')) {
            ...
            $sql = 'SELECT sc.id, c.parent_id, count(*) as total' . '
				...
				WHERE a.country_code = :countryCode AND a.id!=0 AND (a.verified_email=1 AND a.verified_phone=1) AND a.archived!=1 AND a.id!=0 AND a.deleted_at IS NULL
				GROUP BY sc.id';
            $bindings = [
        ...
        $postTypes = Cache::remember($cacheId, $this->cacheExpiration, function () {
            $postTypes = PostType::trans()->orderBy('depth')->get();
            return $postTypes;
        });
