		...
		if (count($files) > 0) {
			foreach ($files as $key => $file) {
				if (empty($file)) {
                    ...
                    $file->move(public_path()."/storage/files/".strtolower(config('country.code'))."/$post->id/", $name);
                // Give the Complete Path of the folder where you want to save the image
                $folder=public_path()."/storage/files/".strtolower(config('country.code'))."/$post->id/";
