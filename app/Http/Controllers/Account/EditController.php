...
class EditController extends AccountBaseController
{
	...
	public function index(Request $request)
	{
		...
		$data['countFavoritePosts'] = SavedPost::whereHas('post', function ($query) {
			...
			->count();
        $cities = explode(',' ,auth()->user()->cities_ids);

        view()->share('cities', json_encode($cities));
		// Meta Tags
...
//            die(var_dump($nearest[0]->id));
	...
	public function updateDetails(UserRequest $request)
	{
		...
		$user->cities_ids=(is_array($request->exhibitions_place) ? implode(",",$request->exhibitions_place) : "");
