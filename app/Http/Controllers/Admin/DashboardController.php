...
class DashboardController extends PanelController
{
	...
	public function __construct()
	{
		...
		// Count Ads
		$countActivatedPosts = Post::where('id','!=',0)->verified()->count();
		$countUnactivatedPosts = Post::where('id','!=',0)->unverified()->count();
	...
	public function dashboard()
	{
		...
		// Get latest Ads
		$this->data['latestPosts'] = Post::take($latestEntriesLimit)->orderBy('id', 'DESC')->where('id','!=',0)->get();
	...
	private function getLatestPostsChart($statDayNumber = 30)
	{
		...
		for ($i = 1; $i <= $statDayNumber; $i++) {
			...
			// Ads Stats
			$countActivatedPosts = Post::where('id','!=',0)->verified()
				->where('created_at', '>=', $date)
				->where('created_at', '<=', $date . ' 23:59:59')
				->count();
			
			$countUnactivatedPosts = Post::where('id','!=',0)->unverified()
				->where('created_at', '>=', $date)
				->where('created_at', '<=', $date . ' 23:59:59')
				->count();
