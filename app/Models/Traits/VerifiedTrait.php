...
trait VerifiedTrait
{
    ...
    public function getVerifiedBookingEmailHtml()
    {

        if (!isset($this->active)) return false;

        // Get checkbox
        $out =ajaxCheckboxDisplay($this->{$this->primaryKey}, $this->getTable(), 'active', $this->active);
        ...
        if (!empty($entity->user_id)) {
            if ($entity->active != 1) {
                // ToolTip
                $toolTip = (!empty($entity->user_id)) ? 'data-toggle="tooltip" title="'. trans('admin::messages.To') . ': ' . $entity->user_id . '"' : '';
                ...
                if($entity->payment_method_id == '2'){

                        // Show send booking verification message link and email
                        $entitySlug = 'user/booking';
                        $urlPath = 'verify/' . $entitySlug . '/' . $entity->user_id .  '/' . $entity->post_id .  '/' . $entity->package_id.  '/' . $entity->id  .'/resend/email' . $localeQueryString;
                        $actionUrl = admin_url($urlPath);
                        // HTML Link
                        $out .= ' &nbsp;';
                        $out .= '<a class="btn btn-default btn-xs" href="' . $actionUrl . '" ' . $toolTip . '>';
