...
class AdsCleaner extends Command
{
	...
	public function handle()
	{
		...
		foreach ($countries as $country) {
			// Ads Query
			$posts = Post::where('id','!=','0')->withoutGlobalScopes([VerifiedScope::class, ReviewedScope::class])->countryOf($country->code);
				...
				if (!isVerifiedPost($post)) {
					...
					if ($post->featured == 1) {
						...
						if ($packages->count() > 0) {
							...
							$payment = Payment::where('post_id', $post->id)->where('package_id','!=',19)->orderBy('id', 'DESC')->first();
							if (!empty($payment)) {
								// Get Package info
