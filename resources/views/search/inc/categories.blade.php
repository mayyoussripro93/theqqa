            ...
            <div class="category-links">
                <ul>
                    @foreach ($subCats->get($cat->tid) as $iSubCat)
                        <li title="{{ $iSubCat->name }}">
                            <?php
                            $attr = ['countryCode' => config('country.icode'), 'catSlug' => $cat->slug, 'subCatSlug' => $iSubCat->slug];
                            ?>
                            <img src="{{ resize($iSubCat->picture, 'x-smoll') }}" alt="img"/>
