                    ...
                    <ul class="nav nav-wizard">
                        @if (getSegment(2) == 'create')
							...
							@if (!in_array($uriPath, ['finish']))
									<li class="{{ (getSegment(2)  == 'req_doc') ? 'active' : (in_array($uriPath, ['photos', 'packages', 'finish']) or (isset($post) and !empty($post)) ? '' : 'disabled') }}">
										@if (isset($post) and !empty($post))
											<a href="{{ lurl('posts/req_doc/' . $post->tmp_token .'') }}">{{ t('Required Documents') }}</a>
										@else
											<a href="{{ lurl('posts/req_doc') }}">{{ t('Required Documents') }}</a>
										@endif
									</li>
                        ...
                        @else
							...
							@if (!in_array($uriPath, ['finish']))
								...
								<li class="{{ (in_array( getSegment(2)=='create', [null, 'edit'])) ? 'active' : '' }}">
									@if (isset($post) and !empty($post))
