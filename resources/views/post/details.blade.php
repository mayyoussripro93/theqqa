...
@section('content')
    ...
    <div class="main-container ad-page">
        ...
        <div class="container">
            <div class="row">
                <div class="col-md-9 col-thin-right">
                    <nav aria-label="breadcrumb" role="navigation" class="pull-left">
                        <ol class="breadcrumb">
                            ...
                            @else
                                <li class="breadcrumb-item">
                                    <?php
                                    $attr = ['countryCode' => config('country.icode'), 'catSlug' => $post->category->slug]; ?>
                                    <a href="{{ lurl(trans('routes.v-search-cat', $attr), $attr) }}">
                                        {{ $post->category->name }}
                                    </a>
                                </li>
                            ...
                            <li class="breadcrumb-item active"
                                aria-current="page">{{ str_limit($post->title, 70) }}</li>
                        </ol>
                    </nav>
                    <a class="ad-code btn btn-success"> {{ t('ad_code').': '.$post->id}} </a>
                </div>
                <div class="col-md-3">
                    <div class="pull-right backtolist">
                        <a href="{{ URL::previous() }}"><i
                                    class="fa fa-angle-double-left"></i> {{ t('Back to Results') }}</a>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="inner inner-box ads-details-wrapper pb-0">
                        ...
                        <span class="info-row">
							...
							<span class="item-location"><i
                                        class="fas fa-map-marker-alt" style="margin-right: 0.2em; margin-left: 0.2em;"></i> {{ $post->city->name }} </span> -&nbsp;
							<span class="category">
                        ...
                        <div class="post-image">
                            ...
                            @else
                                <ul class="bxslider">
                                    <li><img src="{{ resize(config('larapen.core.picture.default'), 'big') }}"
                                             alt="img"></li>
                                </ul>
                                <div class="product-view-thumb-wrapper">
                                    <ul id="bx-pager" class="product-view-thumb">
                                        <li>
                                            <a class="thumb-item-link" data-slide-index="0" href="">
                                                <img src="{{ resize(config('larapen.core.picture.default'), 'small') }}"
                                                     alt="img">
                                            </a>
                                        </li>
                                    </ul>
                        ...
                        <div class="ads-details">
                            <ul class="nav nav-tabs">
                                <li class="nav-item active">
                                    <a class="nav-link" href="#tab-details" data-toggle="tab">
                                        <h4>{{ t('Ad Details') }}</h4></a>
                                </li>
                                @if (config('plugins.reviews.installed'))
                                    <li class="nav-item">
                                        <a class="nav-link" href="#tab-{{ config('plugins.reviews.name') }}"
                                           data-toggle="tab">
                                            <h4>
                                                        ...
                                                        @else
                                                            @if ($post->email != '')
                                                                <a data-toggle="modal" href="{{ $contactSellerURL }}">
                                                                    <i class="icon-mail-2 tooltipHere"
                                                                       data-toggle="tooltip"
                                                                       data-original-title="{{ t('Send a message') }}"></i>
                                                                </a>
                                                            @else
                            ...
                            <div class="content-footer text-left">
                                @if (auth()->check())
                                    @if (auth()->user()->id == $post->user_id)
                                        <a class="btn btn-default" href="{{ lurl('posts/' . $post->id . '/edit') }}"><i
                                                    class="fa fa-pencil-square-o"></i> {{ t('Edit') }}</a>
                                    @else
                                        @if ($post->email != '')
                                            <a class="btn btn-default" data-toggle="modal"
                                               href="{{ $contactSellerURL }}"><i
                                                        class="icon-mail-2"></i> {{ t('Send a message') }} </a>
                                        @endif
                                    @endif
                                @else
                                    @if ($post->email != '')
                                        <a class="btn btn-default" data-toggle="modal" href="{{ $contactSellerURL }}"><i
                                                    class="icon-mail-2"></i> {{ t('Send a message') }} </a>
                                    @endif
                                @endif
...
{{--                                             @endif--}}
                                        @else
                                            ...
                                            @if($post->postType->id == 2 or $post->postType->id == 4 or $post->postType->id == 5 or $post->postType->id == 6  )

                                            @if($post->category->parent_id == 439 or $post->category->parent_id ==440 or $post->category->parent_id == 1 or $post->category->parent_id ==154
                                            or $post->category->id == 439 or $post->category->id ==440 or $post->category->id == 1 or $post->category->id ==154 )
                                                <a href="{{ lurl('contactfor/'.$post->id.'/booking_car') }}"
                                                   class="btn btn-default btn-block">
                                                    <i class="icon-money"></i> {{ t('Booking the car') }}
                                                </a>
                                            @endif
                                                @endif
                                            @if ($post->phone_hidden != 1 and !empty($post->phone))
                                                <a href="{{ $phoneLink }}"
                                                   {!! $phoneLinkAttr !!} class="btn btn-success btn-block showphone">
                                                    <i class="icon-phone-1"></i>
                                                    {!! $phone !!}{{-- t('View phone') --}}
                                                </a>
                        ...
                        @if ($post->post_type_id == '6' or $post->post_type_id == '5')
                            {{--@if ($post->category->id == '154' or $post->category->id == '1' or $post->category->id == '440' or $post->category->id == '439' )--}}
                                <div class="card sidebar-card services-card">
                                    <div class="card-header">
                                        <h4 style="padding-bottom: 0;"><strong>{{ t('services_title_car') }}</strong>
                                        </h4>
                                    </div>
                                    <div class="card-content">
                                        <div class="card-body text-center">
                                            <?php
                                            $url = url()->current();
                                            //                                            Session::put('car_url', $url);
                                            ?>
                                            @if( $post->category->parent_id == 439 or $post->category->parent_id ==440 or $post->category->id == 439 or $post->category->id ==440 )

                                                <a href="{{ lurl('contactfor/ownership')."?u=".urlencode($url)."&p=".$post->title   }}"
                                                   class="btn btn-default service-btn"><img
                                                            src="{{ asset('images/owner-blue.png') }}" width="30">
                                                    <span>{{ t('ownership_title') }}</span></a>
                                                <a href="{{ lurl('contactfor/shipping') ."?u=".urlencode($url)."&p=".$post->title  }}"
                                                   class="btn btn-default service-btn"><img
                                                            src="{{ asset('images/shipping-blue.png') }}" width="30">
                                                    <span>{{ t('shipping_title') }}</span></a>
                                                <a href="{{ lurl('contactfor/mogaz')."?u=".urlencode($url)."&p=".$post->title   }}"
                                                   class="btn btn-default service-btn"><img
                                                            src="{{ asset('images/mogaz-blue.png') }}" width="30">
                                                    <span>{{ t('mogaz_title') }}</span></a>
                                                    <a href="{{ lurl('contactfor/estimation')."?u=".urlencode($url)."&p=".$post->title    }}"
                                                       class="btn btn-default service-btn"><img
                                                                src="{{ asset('images/estimate-blue.png') }}" width="30">
                                                        <span>{{ t('estimation title') }}</span></a>

                                            @else
                                                <a href="{{ lurl('contactfor/checking')."?u=".urlencode($url)."&p=".$post->title  }}"
                                                   class="btn btn-default service-btn"><img
                                                            src="{{ asset('images/check-blue.png') }}" width="30">
                                                    <span>{{ t('checking_title') }}</span></a>
                                                <a href="{{ lurl('contactfor/maintenance') ."?u=".urlencode($url)."&p=".$post->title  }}"
                                                   class="btn btn-default service-btn"><img
                                                            src="{{ asset('images/maintenance-blue.png') }}" width="30">
                                                    <span>{{ t('maintenance_title') }}</span></a>
                                                <a href="{{ lurl('contactfor/ownership')."?u=".urlencode($url)."&p=".$post->title   }}"
                                                   class="btn btn-default service-btn"><img
                                                            src="{{ asset('images/owner-blue.png') }}" width="30">
                                                    <span>{{ t('ownership_title') }}</span></a>
